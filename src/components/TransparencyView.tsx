    ...
    <div className="space-y-12 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-4 flex flex-col justify-between">
          ...
          <span className="text-[10px] font-mono text-gray-400 bg-gray-50 px-2 py-0.5 rounded w-fit text-red-600 font-semibold">
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-gray-100 p-6 shadow-xs space-y-4">
          ...
          <div className="divide-y divide-gray-100 text-xs sm:text-sm">
            <div className="py-3 flex justify-between">
              <span className="font-semibold text-gray-500">{lang === 'en' ? 'Registered Trust Name' : 'பதிவு செய்யப்பட்ட பெயர்'}</span>
              <span className="font-medium text-gray-900 text-right">நல்லதே நடக்கும் சமூக சேவை அறக்கட்டளை</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="font-semibold text-gray-500">{lang === 'en' ? 'Registration Deed Number' : 'அறக்கட்டளை பதிவு எண்'}</span>
              <span className="font-medium text-gray-900 text-right font-mono">Doc No. 16/2025</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="font-semibold text-gray-500">{lang === 'en' ? 'Sub-Registrar Office' : 'சார்பதிவாளர் அலுவலகம்'}</span>
              <span className="font-medium text-gray-900 text-right">Tiruchengode Town & Taluk, Namakkal</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="font-semibold text-gray-500">{lang === 'en' ? 'Date of Registry' : 'பதிவு செய்யப்பட்ட தேதி'}</span>
              <span className="font-medium text-gray-900 text-right font-mono">08.04.2025</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="font-semibold text-gray-500">{lang === 'en' ? 'Drafting Advocate' : 'ஆவணத்தை வடிவமைத்த வழக்கறிஞர்'}</span>
              <span className="font-medium text-gray-900 text-right">Advocate Sathiskumar, Tiruchengode</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="font-semibold text-gray-500">{lang === 'en' ? '12A & 80G Tax Exemption Status' : '12A மற்றும் 80G வரி விலக்கு நிலை'}</span>
            ...
            {auditLogs.map((log, idx) => (
              <div key={idx} className="p-4 border border-gray-100 rounded-xl bg-gray-50/50 flex items-start justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-gray-500">{log.auditor[lang]}</p>
                </div>
              ...
              <div key={idx} className="py-4">
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      <p className="text-xs sm:text-sm text-gray-600 leading-relaxed text-justify pt-3 pr-6 text-slate-500">
                        {faq.answer[lang]}
                      </p>
                    </motion.div>
                  )}
